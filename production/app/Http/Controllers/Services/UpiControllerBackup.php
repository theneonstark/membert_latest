...
class UpiControllerBackup extends Controller
{
    protected $admin, $api;
    ...
    public function create(Request $post)
    {
        ...
        switch ($api->code) {
            ...
            case 'atmoondpspayin':
                $url  = "https://atmoondps.com/api/Atmoondigitalpaymentsolution";
                $parameter = [
                    "method" => "PAYIN",
                    ...
                    "payment_mode" => "INTENT",
                    "payer_vpa" => "TESTVPA",
                    "callback_url" => "https://api.peunique.com/production/api/webhook/atmoondps/payin",
                ...
                $header = array(
                    'Content-Type: application/json',
                    'user_id: 228',
                    'utoken: 939088281001198'
                );
            
                $result   = \App\Helpers\Permission::curl($url, 'POST', json_encode($parameter), $header, "yes", "Qr", $post->txnid); 
                $response = json_decode($result['response']);

                if(isset($response->response->data->intent_url)){
                    $post['type']  = "upigateway";
                    $post['refId'] = $response->response->data->transactionId;
                    $post['txnid'] = $response->response->data->order_id;
                    $post['upi_string'] = str_replace(" ", "", $response->response->data->intent_url);
                    $post['upi_string_image'] = "https://chart.googleapis.com/chart?chs=300x300&cht=qr&chl=".urlencode($response->response->data->intent_url);  
            ...
            case 'payincpayin':
                $url  = "https://api.payinc.in/api/v1/token/generate";
                
                $parameter = [
                    "UserRMN" => "7982724172",
                    "UserPassword" => "45615721"
                ];
                ...
                if(isset($response->payload->token)){
                    ...
                    if($user->payinid == "no"){
                        $url = "https://api.payinc.in/api/v1/corppayout/agent/add";
                        $parameter = [
                            "AgentMobileNumber" => $user->mobile,
                            "AgentName"         => $user->name,
                            "AgentShopName"     => $user->shopname,
                            "AgentAddress"      => $user->address,
                            "AgentCityName"     => $user->city,
                            "AgentStateName"    => $user->state,
                            "AgentPinCode"      => $user->pincode,
                            "AgentPanCard"      => $user->pancard,
                            "AgentGstIn"        => $user->gstin,
                            "AgentEmailId"      => $user->email,
                        ];

                        $header = array(
                            'Content-Type: application/json',
                            'Authorization: '.$response->payload->token
                        );
                        
                        $result   = \App\Helpers\Permission::curl($url, 'POST', json_encode($parameter), $header, "yes", "Onboard", $user->mobile); 
                        $response = json_decode($result['response']);

                        if(isset($response->AgentData->AgentId)){
                            \DB::table("users")->where("user_id", $post->user_id)->update(["payinid" => $response->AgentData->AgentId]);
                        }else{
                            return response()->json(['statuscode' => "TXF", "message" => isset($response->Message)?$response->Message:"Something went wrong"]);
                        }
                    }
    ...
    public function webhook(Request $post)
    {
        ...
        if($post->status == "success"){
            $payinReport = Collectionreport::where("txnid", $post->apitxnid)->where("status", "pending")->where("product", "qrcode")->first();
            ...
            if($payinReport){
                ...
                try {
                    $report = \DB::transaction(function () use($insert, $api, $wallet, $table, $reportTable, $payinReport){
                        ...
                        if($api->code != "collect"){
                            \DB::table("collectionreports")->where('id', $payinReport->id)->where("product", "qrcode")->update([
                                "status"  => "success",
                                "refno"   => $insert["refno"]
                            ]);
                        }
    ...
    public function indicpay(Request $data)
    {
        ...
        if($post->status == "SUCCESS"){
            $payinReport = Collectionreport::where("txnid", $post->txnid)->where("status", "pending")->where("product", "qrcode")->first();
            ...
            if($payinReport){
                ...
                $data['charge'] = \App\Helpers\Permission::getCommission($amount[0], $payinReport->user->scheme_id, $provider->id, $payinReport->user->role->slug);
                ...
                try {
                    ...
                    if($api->code != "collect"){
                        $webhook_payload["amount"] = $amount[0];
                        $webhook_payload["status"] = "success";
                        $webhook_payload["statuscode"] = "TXN";
                        $webhook_payload["txnid"]  = $payinReport->txnid;
                        $webhook_payload["apitxnid"]  = $payinReport->apitxnid;
    ...
    public function pinwallet(Request $data)
    {
        ...
        if(($mydata["event"] == "DynamicUPIQR" || $mydata["event"] == "DynamicUPI") &&  $data['TxnStatus'] == "SUCCESS"){
            $payinReport = Collectionreport::where("txnid", $txnid)->where("status", "pending")->where("product", "qrcode")->first();
    ...
    public function ekopayin(Request $post)
    {
        if($post->tx_status == 0){
            ...
            $payinReport = Collectionreport::where("txnid", $post->client_ref_id)->where("status", "pending")->where("product", "qrcode")->first();
    ...
    public function atmoondpspayin(Request $post)
    {
        $log = \DB::table('log_webhooks')->insert([
            'txnid'      => $post->order_id, 
            'product'    => 'atmoondpspayin', 
            'response'   => json_encode($post->all()),
            "created_at" => date("Y-m-d H:i:s")
        ]);

        if($post->order_status == "PAID"){
            $payinReport = Collectionreport::where("txnid", $post->order_id)->where("status", "pending")->where("product", "qrcode")->first();

            if($payinReport){
                if($post->order_amount > 0 && $post->order_amount <= 199){
                    $provider = Provider::where('recharge1', 'qrcollection1')->first();
                }elseif($post->order_amount > 199 && $post->order_amount <= 499){
                    $provider = Provider::where('recharge1', 'qrcollection2')->first();
                }elseif($post->order_amount > 499 && $post->order_amount <= 999){
                    $provider = Provider::where('recharge1', 'qrcollection3')->first();
                }else{
                    $provider = Provider::where('recharge1', 'qrcollection4')->first();
                }
                ...
                $post['charge'] = \App\Helpers\Permission::getCommission($post->order_amount, $payinReport->user->scheme_id, $provider->id, $payinReport->user->role->slug);
                $post['gst']    = ($post->charge * 18)/100;

                $insert = [
                    'number'  => $post->payer_vpa,
                    'mobile'  => $payinReport->user->mobile,
                    'provider_id' => $provider->id,
                    'api_id'  => $api->id,
                    ...
                    'txnid'   => $payinReport->txnid,
                    'apitxnid'   => $payinReport->apitxnid,
                    'payid'   => $payinReport->payid,
                    'refno'   => $post->bank_reference,
                    'option1' => $post->payment_method,
                    'status'  => 'success',
                    'transfer_mode' => 'callback',
                    'user_id' => $userid,
                    'credit_by'   => $payinReport->user_id,
                    'rtype'       => 'main',
                    'create_time' => $post->bank_reference,
                    'trans_type'  => "credit",
                    'product'     => "payin"
                ];
                ...
                try {
                    ...
                    if($api->code != "collect"){
                        $webhook_payload["amount"] = $post->order_amount;
                        $webhook_payload["status"] = "success";
                        $webhook_payload["statuscode"] = "TXN";
                        $webhook_payload["txnid"]  = $payinReport->txnid;
                        $webhook_payload["apitxnid"]  = $payinReport->apitxnid;
                        $webhook_payload["utr"]    = $post->bank_reference;
                        $webhook_payload["payment_mode"] = $post->payer_vpa;
                        $webhook_payload["payid"]  = $payinReport->id;
                        $webhook_payload["message"]= $post->payment_method;
                        $response = \App\Helpers\Permission::curl($payinReport->remark."?".http_build_query($webhook_payload), "GET", "", [], "no", "no", "no", "no");
    ...
    public function cosmospayin(Request $post)
    {
        $log = \DB::table('log_webhooks')->insert([
            'txnid'      => date("ymdhis"), 
            'product'    => 'cosmospayin', 
